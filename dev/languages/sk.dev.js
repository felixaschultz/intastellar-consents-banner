(function () {
    ...
    var arrangeWord = (window.INTA && window.INTA.settings && window.INTA.settings.arrange == "ltr") ? "ľavom" : "pravom";
    window.__intaCmpLocalePayload = {
        ...
        bannerMessage: "Prijatím všetkých súborov cookie podporujete " + document.domain + " pri vývoji lepšieho riešenia pre vás.</p><p>Vyberte, či chcete povoliť iba nevyhnutné súbory cookie, alebo povoliť všetky súbory cookie.",
        settingsMessage: "<h3 style=\"    font-size: 25px;\">Máte to pod kontrolou</h3>"
            + "<p>My aj naši dôveryhodní partneri používame technológie, ako sú súbory cookie, na zhromažďovanie informácií na rôzne účely, okrem iného:</p>"
            + "<ol>"
            + "<li>Funkčné</li>"
            + "<li>Analytika</li>"
            + "<li>Marketing</li>"
            + "</ol>"
            + "<p>Kliknutím na „Prijať“ súhlasíte so všetkými týmito účelmi. Prípadne si môžete vybrať konkrétne účely, s ktorými súhlasíte, zaškrtnutím políčok a kliknutím na „Uložiť nastavenia“.</p>"
            + "<p>Svoj súhlas môžete kedykoľvek odvolať kliknutím na malú ikonu v " + arrangeWord + " dolnom rohu webovej stránky.</p>",
        settingsMessageSuffix: '<button onClick="showPrivacy()" class="intastellarCookie-settings__privacyLink">Zásady ochrany osobných údajov Intastellar Solutions, International</button>',
