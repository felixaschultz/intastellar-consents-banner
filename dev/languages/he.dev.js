(function () {
    ...
    var arrangeWord = (window.INTA && window.INTA.settings && window.INTA.settings.arrange == "ltr") ? "שמאלית" : "ימנית";
    window.__intaCmpLocalePayload = {
        ...
        bannerMessage: "באישור כל העוגיות, אתה תומך ב-" + document.domain + " בפיתוח פתרון טוב יותר עבורך.</p><p>בחר האם ברצונך לאפשר רק את העוגיות ההכרחיות או לאפשר את כל העוגיות.",
        settingsMessage: "<h3 style=\"    font-size: 25px;\">השליטה בידיך</h3>"
            + "<p>אנו והשותפים המהימנים שלנו משתמשים בטכנולוגיות, כגון עוגיות, לאיסוף מידע למטרות שונות, ובכללן:</p>"
            + "<ol>"
            + "<li>פונקציונלי</li>"
            + "<li>אנליטיקה</li>"
            + "<li>שיווק</li>"
            + "</ol>"
            + "<p>בלחיצה על 'אישור', אתה מסכים לכל המטרות הללו. לחלופין, תוכל לבחור את המטרות הספציפיות שאתה מסכים להן על ידי סימון התיבות ולחיצה על 'שמור הגדרות'.</p>"
            + "<p>תוכל למשוך את הסכמתך בכל עת על ידי לחיצה על הסמל הקטן בפינה ה" + arrangeWord + " התחתונה של האתר.</p>",
        settingsMessageSuffix: '<button onClick="showPrivacy()" class="intastellarCookie-settings__privacyLink">מדיניות הפרטיות של Intastellar Solutions, International</button>',
